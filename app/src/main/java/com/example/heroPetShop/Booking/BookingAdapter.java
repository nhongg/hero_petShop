...
public class BookingAdapter extends RecyclerView.Adapter<BookingAdapter.BookingViewHolder> {
    ...
    public static void showOptionsDialog(Context context, Booking booking) {
...
//                            editBooking(context, booking);
    ...
    // Hủy đặt lịch
    private static void cancelBooking(Context context, Booking booking) {
        if (booking.getIdBooking() == null || booking.getIdBooking().isEmpty()) {
            Toast.makeText(context, "Không thể xác định lịch để hủy", Toast.LENGTH_SHORT).show();
            return;
        }

        FirebaseFirestore db = FirebaseFirestore.getInstance();
        db.collection("bookings").document(booking.getIdBooking())
